...
pkgs.mkShell {
  nativeBuildInputs = with pkgs; [ gnumake gcc emscripten pkg-config gdb clang-tools ];
  buildInputs = with pkgs; [ xorg.libX11 libGL libpng alsa-lib lua5_4 ]; # Everything -lX11 -lGL -lpng -lasound -llua needs
  ...
  shellHook = ''
    ...
    echo "targets: make all | make dev | make debug | make run"
  '';
}
