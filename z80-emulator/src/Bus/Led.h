...
public:
  ...
  void Draw(PixelGameEngine* GameEngine) {
    for (int32_t i = 0, data = this->data; i < 8; i++, data = data << 1) {
      ...
      if (data & 0x80) GameEngine->FillRect(pos, olc::vi2d(8, 8), ~AnyType<WHITE, ColorT>::GetValue());
      else GameEngine->FillRect(pos, olc::vi2d(8, 8), ~AnyType<DARK_GREY, ColorT>::GetValue());
    }
  }
